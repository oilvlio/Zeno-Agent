...
func newAgentTestClient(baseURL, nodeID, token string) *agent.Client {
	return agent.NewClientWithOptions(baseURL, nodeID, token, agent.ClientOptions{})
}
...
func TestRefreshProbeTargetsRequiresRequestedConfigVersion(t *testing.T) {
	...
	client := newAgentTestClient(server.URL, "node-a", "token")
